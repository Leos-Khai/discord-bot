...
**Playback Position**:
The point within the current track that Guild Playback has reached. It advances while playing, holds while paused, and moves to a requested point when sought.
_Avoid_: elapsed time, playback start time

**Seek Request**:
A requested change to Playback Position, given either as an absolute point in the track or as a shift forward or backward from the current position. A request landing before the track's start begins the track again from its start; one landing at or beyond the track's end is refused, as is any request against a track whose length is unknown.
_Avoid_: scrub, jump, fast-forward
